...
import bitcoinBaron from '../../static/audio/ytcracker - bitcoin-baron.m4a';
...
import derezzed from '../../static/audio/daft-punk-derezzed.mp3';
import derezzedCover from '../../static/images/album-covers/daft-punk-derezzed.png';
...
const TRACKS: Track[] = [
  ...
  { id: 8,  title: 'Derezzed',         artist: 'Daft Punk', album: 'TRON: Legacy Soundtrack', producer: 'Wonderland Music Company, Inc.', src: derezzed, cover: derezzedCover, links: { spotify: 'https://open.spotify.com/artist/4tZwfgrHOc3mvqYlEYSvVi' } },
];
...
export default function NowPlaying({ isOpen, onClose, onExpand }: Props) {
  ...
  // Tracks whether onClose was triggered by minimize (vs. a real close)
  ...
  const expand = () => {
    isMinimizingRef.current = true; // signal: don't reshuffle on re-open
    setMinimized(false);
    onExpand();
  };
  ...
  useEffect(() => {
    if (isOpen) {
      if (!isMinimizingRef.current) {
        // Fresh open — shuffle and start from top
      ...
      if (!isMinimizingRef.current) {
        // Real close (not a minimize) — stop audio
        stopAudio();
      }
    }
  }, [isOpen, stopAudio]);
  ...
  // Mini-player — shown while minimized regardless of isOpen
  const miniPlayer = minimized && !isOpen && (
    <div className="np-mini">
      <button className="np-mini-speaker" onClick={expand} aria-label="Expand player">
        <svg className={`np-mini-icon ${playing ? 'playing' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/>
          <path className="np-mini-wave1" d="M15.54 8.46a5 5 0 0 1 0 7.07"/>
          <path className="np-mini-wave2" d="M19.07 4.93a10 10 0 0 1 0 14.14"/>
        </svg>
      </button>
      <div className="np-mini-info" onClick={expand}>
        <span className="np-mini-title">{track.title}</span>
        <span className="np-mini-sep">—</span>
        <span className="np-mini-artist">{track.artist}</span>
      </div>
      <div className="np-mini-progress">
        <div className="np-mini-progress-fill" style={{ width: `${progress * 100}%` }} />
      </div>
      <button className="np-mini-ctrl" onClick={toggle} aria-label={playing ? 'Pause' : 'Play'}>
        {playing ? '⏸' : '▶'}
      </button>
      <button className="np-mini-close" onClick={fullClose} aria-label="Stop and close">✕</button>
    </div>
  );

  if (!isOpen) return <>{miniPlayer}</>;

  return (
    <>
      {miniPlayer}
      <div className="np-overlay" onClick={minimize}>
        <div className="np-modal" onClick={e => e.stopPropagation()}>
          ...
          {/* Minimize and Close */}
          <button className="np-minimize" onClick={minimize} aria-label="Minimize">−</button>
          <button className="np-close" onClick={fullClose} aria-label="Close">✕</button>
        </div>
      </div>
    </>
  );
